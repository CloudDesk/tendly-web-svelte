<script lang="ts">
  ...
  import MyResignation from "$lib/components/employee/MyResignation.svelte";
  ...
  const announcements = [
    { title: "Office Renovation", content: "The 3rd floor will be under renovation from May 5-12.", date: "Apr 18" },
    { title: "New Benefits Package", content: "Updated health benefits package details now available in HR portal.", date: "Apr 15" },
    { title: "Happy Birthday!", content: "Wishing Mark Johnson a very happy birthday today!", date: "Apr 19" }
  ];
...
<div class="p-4 md:p-6 bg-gray-50 min-h-screen">
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
    ...
    <div class="col-span-1 md:col-span-2 bg-white rounded-lg shadow-sm overflow-hidden border border-gray-100">
      <SwipesTracking />
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm p-4 border border-gray-100">
      <div class="flex items-center justify-between mb-4">
        ...
        <span class="bg-blue-100 text-blue-800 text-xs py-1 px-2 rounded">Latest: {payslipInfo.latest}</span>
      </div>
      
      <div class="bg-gradient-to-r from-blue-50 to-indigo-50 p-3 rounded-md mb-4">
      ...
      <div class="flex space-x-2">
        <button class="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-2 px-3 rounded-md text-sm flex items-center justify-center">
          <FileText size={16} class="mr-1" />
          Download
        </button>
        <button class="flex-1 border border-gray-300 hover:bg-gray-50 text-gray-700 py-2 px-3 rounded-md text-sm">
          Salary History
        </button>
      </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm overflow-hidden border p-4 border-gray-100">
      <h2 class="text-lg font-semibold text-gray-800 flex items-center">
    ...
    <div class="bg-white rounded-lg shadow-sm p-4 border border-gray-100">
      <div class="flex items-center justify-between mb-4">
        ...
        <button class="bg-green-600 hover:bg-green-700 text-white py-1 px-3 rounded-md text-xs flex items-center">
          Apply Leave
        </button>
      </div>
      ...
      <div class="grid grid-cols-4 gap-2 mb-4">
        <div class="bg-blue-50 p-2 rounded text-center">
          <div class="text-lg font-bold text-blue-700">{leaveBalance.casual}</div>
          <div class="text-xs text-gray-600">Casual</div>
        </div>
        <div class="bg-green-50 p-2 rounded text-center">
          <div class="text-lg font-bold text-green-700">{leaveBalance.sick}</div>
          <div class="text-xs text-gray-600">Sick</div>
        </div>
        <div class="bg-purple-50 p-2 rounded text-center">
          <div class="text-lg font-bold text-purple-700">{leaveBalance.earned}</div>
          <div class="text-xs text-gray-600">Earned</div>
        </div>
        <div class="bg-gray-50 p-2 rounded text-center">
          <div class="text-lg font-bold text-gray-700">{leaveBalance.total}</div>
          ...
          {#each upcomingLeaves as leave}
            <div class="flex items-center justify-between py-1">
              <div>
                ...
                <span class="text-xs ml-2 bg-blue-100 text-blue-800 py-0.5 px-1.5 rounded">{leave.type}</span>
              </div>
              <span class="text-xs text-green-600 font-medium">{leave.status}</span>
            </div>
          {/each}
        </div>
      {:else}
    ...
    <div class="bg-white rounded-lg shadow-sm p-4 border border-gray-100">
      ...
      <div class="bg-gradient-to-r from-blue-50 to-indigo-50 p-4 rounded-md">
        <div class="text-center">
          <h3 class="text-lg font-semibold text-blue-800">{currentShift.name}</h3>
