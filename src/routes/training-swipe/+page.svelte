<script lang="ts">
  ...
  async function handleSwipe() {
    ...
    try {
      const response = await fetch(`${base_url}/training-attendance/swipe`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error('Failed to record training attendance');
      }
      ...
      messageType = 'error';
      message = 'Failed to record training attendance';
    }
