...
<div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
  <div class="flex items-center justify-between mb-6">
    ...
    {#if payslipData}
      <span
        class="bg-blue-100 text-blue-800 text-sm py-1 px-3 rounded-full font-medium"
      >
        ...
        <div class="space-y-3">
          <div class="flex justify-between items-center">
            ...
            <span class="font-semibold text-gray-800"
              >₹{payslipData.basic.toLocaleString()}</span
            >
          </div>
          <div class="flex justify-between items-center">
            <span class="text-gray-600">HRA</span>
            <span class="font-semibold text-gray-800"
              >₹{payslipData.hra.toLocaleString()}</span
            >
          </div>
          <div class="flex justify-between items-center">
            <span class="text-gray-600">Other Allowance</span>
            <span class="font-semibold text-gray-800"
              >₹{payslipData.otherAllowance.toLocaleString()}</span
            >
          </div>
          <div class="border-t border-gray-100 pt-3 mt-3">
            <div class="flex justify-between items-center">
              ...
              <span class="font-bold text-blue-600"
                >₹{payslipData.grossSalary.toLocaleString()}</span
              >
            </div>
            <div class="flex justify-between items-center mt-2">
              <span class="text-gray-600">Total Deductions</span>
              <span class="font-bold text-red-600"
                >₹{payslipData.totalDeductions.toLocaleString()}</span
              >
            </div>
            <div class="flex justify-between items-center mt-2">
              <span class="text-gray-600">Net Salary</span>
              <span class="font-bold text-green-600"
                >₹{payslipData.netSalary.toLocaleString()}</span
              >
            </div>
          </div>
        </div>
