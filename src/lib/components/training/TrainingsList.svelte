<script lang="ts">
  ...
  async function handleSubmit() {
    try {
      ...
      if (editingTraining._id) {
        ...
        //   ...updateData
        // } = editingTraining;
        await trainingsApi.update(editingTraining._id, editingTraining);
      } else {
        await trainingsApi.create({ ...editingTraining } as Omit<
          Training,
          "_id"
        >);
      }
  ...
  async function handleAssignmentSubmit() {
    try {
      ...
      await trainingsApi.assignEmployees(
      ...
      try {
        ...
        editingTraining = {
          ...
          validFrom: data.validFrom
            ? new Date(data.validFrom).toLocaleDateString("en-CA")
            : "",
          validTill: data.validTill
            ? new Date(data.validTill).toLocaleDateString("en-CA")
            : "",
        };
        showForm = true;
      } catch (err) {
...
>
  <form on:submit|preventDefault={handleSubmit} class="space-y-4">
    <div class="form-control">
      <label class="label" for="training-name">Name</label>
      <input
        id="training-name"
        type="text"
        class="input input-bordered"
        bind:value={editingTraining.title}
        required
      />
    </div>

    <div class="form-control">
      <label class="label" for="training-code">Code</label>
      <input
        id="training-code"
        type="text"
        class="input input-bordered"
    ...
    <div class="grid grid-cols-2 gap-4">
      <div class="form-control">
        <label class="label" for="training-start">Start Time</label>
        <input
          id="training-start"
          type="time"
          class="input input-bordered"
    ...
    <div class="grid grid-cols-2 gap-4">
      <div class="form-control">
        <label class="label" for="training-window-start"
          >Training Window Start</label
        >
        <input
          id="training-window-start"
          type="time"
          class="input input-bordered"
      ...
      <div class="form-control">
        <label class="label" for="training-window-end"
          >Training Window End</label
        >
        <input
          id="training-window-end"
          type="time"
          class="input input-bordered"
    ...
    <div class="grid grid-cols-2 gap-4">
      <div class="form-control">
        <label class="label" for="training-from">Valid From</label>
        <input
          id="training-from"
          type="date"
          class="input input-bordered"
          bind:value={editingTraining.startDate}
          required
        />
      </div>

      <div class="form-control">
        <label class="label" for="training-till">Valid Till</label>
        <input
          id="training-till"
          type="date"
          class="input input-bordered"
          bind:value={editingTraining.endDate}
        />
      </div>
    </div>

    <div class="form-control">
      <label class="label" for="grace-time">Grace Time (minutes)</label>
      <input
        id="grace-time"
        type="number"
        class="input input-bordered"
    ...
    <div class="grid grid-cols-2 gap-4">
      <div class="form-control">
        <label class="label" for="max-participants">Max Participants</label>
        <input
          id="max-participants"
          type="number"
          class="input input-bordered"
  ...
  {#if selectedTraining}
    <div class="space-y-4">
      <div class="grid grid-cols-2 gap-4">
        ...
        <div>
          <label class="font-semibold" for="code">Code:</label>
          <p>{selectedTraining.description}</p>
        </div>
      </div>

      <div>
        <label class="font-semibold" for="timing">Timing:</label>
        <p>{selectedTraining.startTime} - {selectedTraining.endTime}</p>
      </div>

      <div>
        <label class="font-semibold" for="window">Window:</label>
        <p>
          {selectedTraining.trainingWindowStart} - {selectedTraining.trainingWindowEnd}
        </p>
      </div>

      <div>
        <label class="font-semibold" for="validity">Validity:</label>
        <p>
      ...
      <div>
        <label class="font-semibold" for="grace">Grace Time:</label>
        <p>{selectedTraining.graceTimeInMinutes || 0} minutes</p>
      </div>

      <div>
        <label class="font-semibold" for="max-participants"
...
<Modal
  ...
  title={`Assign ${selectedTraining?.title || "Training"}`}
  onClose={() => {
    showAssignModal = false;
    assignmentStep = 1;
    selectedEmployees.clear();
...
>
  ...
  {:else}
    <div class="space-y-4">
      <div class="bg-base-200 rounded-lg p-4">
        ...
        <div class="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
          <div>
            ...
            <span class="ml-1">{selectedTraining?.title}</span>
          </div>
          <div>
            <span class="text-base-content/70">Code:</span>
            <span class="ml-1">{selectedTraining?.description}</span>
          </div>
          <div>
            <span class="text-base-content/70">Timing:</span>
            <span class="ml-1"
              >{selectedTraining?.startTime} - {selectedTraining?.endTime}</span
            >
          </div>
          <div>
            <span class="text-base-content/70">Window:</span>
            <span class="ml-1"
              >{selectedTraining?.trainingWindowStart} - {selectedTraining?.trainingWindowEnd}</span
            >
          </div>
          <div>
          ...
          <div>
            ...
            <span class="ml-1"
              >{selectedTraining?.graceTimeInMinutes || 0} minutes</span
            >
          </div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-2 gap-4">
        <div class="form-control">
          <label class="label" for="assign-valid-from"
            >Assignment Valid From</label
          >
          <input
            type="date"
            class="input input-bordered"
        ...
        <div class="form-control">
          <label class="label" for="assign-valid-till"
            >Assignment Valid Till</label
          >
          <input
            type="date"
            class="input input-bordered"
            ...
            <div class="p-3">
              ...
              <p class="text-sm text-base-content/70">
                ...
                {employee.upcomingShiftAssignmentData
                  ? `Current: ${employee.upcomingShiftAssignmentData.shiftCode}`
                  : "No current shift"}
              </p>
            </div>
          {/each}
      ...
      <div class="flex justify-between items-center">
        <button class="btn btn-ghost" on:click={() => (assignmentStep = 1)}>
          Back
        </button>
        <button
          class="btn btn-primary"
