<script lang="ts">
  ...
  let filters = writable<LeaveFilters>({
    ...
    sortOrder: getUrlParam($page.url.searchParams, "sortOrder") as
      | "asc"
      | "desc"
      | undefined,
  });
  ...
  async function updateFiltersAndFetch(
    ...
    filters.set({
      ...
      sortOrder: getUrlParam(url.searchParams, "sortOrder") as
        | "asc"
        | "desc"
        | undefined,
    });
  ...
  async function fetchLeaves() {
    ...
    try {
      ...
      const response = await leavesApi.getLeavesByAssignedId(
        user._id,
        cleanFilters
      );
