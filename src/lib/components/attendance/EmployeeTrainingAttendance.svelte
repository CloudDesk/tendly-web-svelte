<script lang="ts">
  ...
  async function fetchAttendance() {
    ...
    try {
      if (mode === "single") {
        const response: any = await trainingAttendanceApi.search({
          userIds: [employeeId || ""],
        ...
        const response: any = await trainingAttendanceApi.searchAll({
          startDate: toUTCString(startDate),
          endDate: toEndOfDayUTCString(endDate),
        });
        trainingAttendanceData = response.data;
            ...
            <tbody>
              {#each data.records as record}
                <tr class:late={record.attendanceStatus.includes("Late")}>
                  ...
                  <td
                    >{record.lastSwipe !== record.firstSwipe
                      ? new Date(record.lastSwipe).toLocaleTimeString()
                      : "-"}</td
                  >
                  <td>
                    <div class="flex flex-col gap-1">
                      ...
                      {#if record.attendanceStatus.length > 0}
                        <span class="text-xs text-error"
                          >{record.attendanceStatus.join(", ")}</span
                        >
                      {/if}
                    </div>
                  </td>
                ...
                <tr>
                  ...
                  {#each datesInRange as dt}
                    {@const record = data.records.find(
                      (r) =>
                        r.trainingDay.substring(0, 10) === dt.substring(0, 10)
                    )}
                    <td>
                      <div class="relative group">
                        <div
                          class="attendance-card {record
                            ? record.status
                            : 'empty'} {record?.attendanceStatus?.includes(
                            'Late'
                          )
                            ? 'late'
                            : ''}"
                          ...
                          {#if record}
                            ...
                            {#if record.overtime !== "0:00:00"}
                              <span class="text-xs text-success"
                                >+{record.overtime}</span
                              >
                            {:else if record.shortTime !== "0:00:00"}
                              <span class="text-xs text-error"
                                >-{record.shortTime}</span
                              >
                            {/if}
                            <div class="tooltip">
                              <div class="text-xs">
