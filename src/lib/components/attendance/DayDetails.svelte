...
<div
  class="bg-white p-6 rounded-2xl shadow-lg border border-gray-200 h-full space-y-6"
>
  ...
  {#if record}
    <div class="space-y-4">
      ...
      {#if record.needsRegularization && !isToday}
        <div
          class="bg-yellow-50 text-yellow-800 p-4 rounded-md border border-yellow-300"
        >
          <span class="font-semibold">Needs Regularization</span>
        </div>
        {#if record.outOfWindowSwipes && record.outOfWindowSwipes?.length > 0}
          <div
            class="bg-red-50 text-red-600 p-4 rounded-md border border-red-300"
          >
            <div class="font-semibold">
              Reason: {record.outOfWindowSwipes[0]?.reason}
            </div>
            <span
              >Check-in: {record.outOfWindowSwipes[0]?.timestamp
                ? format(
                    new Date(record.outOfWindowSwipes[0]?.timestamp),
                    "hh:mm a"
                  )
                : "-"}</span
            >
