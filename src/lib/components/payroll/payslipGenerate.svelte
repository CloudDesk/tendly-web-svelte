<script lang="ts">
  ...
  interface Employee {
    ...
    payslipStatus?: "Generated" | "Sent" | "Exported" | null;
  ...
  async function fetchPayslipStatus(
    ...
    try {
      ...
      //   Number(month)
      // );
      const response: any = await documentsApi.getUserPayslipStatus(
        userIds,
        Number(year),
        Number(month)
      );
  ...
  function toggleSelectAll() {
    ...
    if (selectAll) {
      ...
      selectedEmployees = new Set(
        employees
          .filter((emp) => emp.payrollStatus === "Completed")
          .map((emp) => emp._id)
      );
    } else {
...
console.log(selectedEmployees,"selectedEmp")
    ...
    selectAll =
      selectedEmployees.size ===
        employees.filter((emp) => emp.payrollStatus === "Completed").length &&
      employees.length > 0;
  }
  ...
  async function generatePayslip() {
    ...
    try {
      ...
      const response = await documentsApi.generatePayslip(payload);
                ...
                disabled={employees.length === 0 ||
                  ...
                  employees.every(
                    (emp) =>
                      ...
                      ["Generated", "Sent", "Exported"].includes(
                        emp.payslipStatus ?? ""
                      )
                  )}
              />
          ...
          {:else}
            {#each employees as emp}
              <tr class="even:bg-gray-50 hover:bg-gray-50 transition">
                ...
                >
                <td class="px-4 py-3 text-gray-600 border-b">
                  {emp.paymentConfirmedAt
                    ? formatDate(emp.paymentConfirmedAt)
                    : "-"}
                </td>
                ...
                <td class="px-4 py-3 text-gray-600 border-b text-center">
                  <input
                    ...
                    class="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                    disabled={emp.payrollStatus !== "Completed" ||
                      ["Generated", "Sent", "Exported"].includes(
                        emp.payslipStatus ?? ""
                      )}
                  />
